...
def combine_trk_cat(split, dataset, results_title, method, suffix, num_hypo):

	# load dataset-specific config
	file_path = os.path.dirname(os.path.realpath(__file__))
	root_dir = os.path.join(file_path, '../../results', dataset, results_title)
	...
	for hypo_index in range(num_hypo):
		...
		for seq_tmp in seq_list:
			...
			for subset_tmp in subset:
				...
				if not is_path_exists(file_tmp):
					print('%s file not exist' % file_tmp)
					continue
