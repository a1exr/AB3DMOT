...
import os
import numpy as np
from scripts.nuScenes.nusc_tracking.CP_KF_tracker import CP_KF_Tracker as Tracker
...
def save_first_frame(args):
    ...
    res_dir = os.path.join(args.work_dir, f'KF_TH={args.TH:.4f}')
...
def eval_tracking(args):
    # args = parse_args()
    kf_res_dir = os.path.join(args.work_dir, f'KF_TH={args.TH:.4f}')
    eval_track_folder = os.path.join(kf_res_dir, 'eval_track')
    os.makedirs(eval_track_folder, exist_ok=True)

    eval(os.path.join(kf_res_dir, f'track_results_nusc.json'),
        args.version,
        eval_track_folder,
        args.root
    )
